...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccr}
    ...
    \midrule
    Gradient Boosting & 0.8245 & 0.8492 & 0.9323 & \textbf{0.8887} & \textbf{0.9106} & 0.364 \\
...
Gradient Boosting achieved the highest F1-Score (0.8887) and ROC-AUC (0.9106),
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Figures/final_performance_comparison.png}
  \caption{Grouped bar chart comparing Precision, Recall, Accuracy, and F1-Score
           across Static Analysis baseline, Random Forest, Gradient Boosting, and
           SVM classifiers. Gradient Boosting leads on F1 and AUC.}
  \label{fig:metrics_bar}
\end{figure}

The performance comparison chart (Figure~\ref{fig:metrics_bar}) highlights the dramatic
improvement from the Static Analysis baseline (Precision~$\approx$~0.60) to the ML
classifiers. Gradient Boosting consistently leads on F1-Score and ROC-AUC. Random Forest
offers a strong second-place result with a higher accuracy score. SVM achieves competitive
precision but at the cost of recall, making it less suitable for a security setting where
missing real vulnerabilities is costly.
...
positives, recall 0.9323) with only a small fraction of safe code mistakenly flagged (128
false positives), drastically outperforming the static baseline. SVM has the highest
...
the optimal threshold at \textbf{0.414} for the Random Forest model.
...
\begin{figure}[htbp]
  ...
  \caption{Confidence Score Analysis for the Gradient Boosting model. The optimal
           decision threshold is \textbf{0.364}, tuned to heavily favour Recall
           and catch the maximum number of true vulnerabilities.}
  \label{fig:conf_gbm}
\end{figure}

Figure~\ref{fig:conf_gbm} shows the Gradient Boosting confidence analysis. The
GBM threshold is optimized at \textbf{0.364} --- lower than Random Forest's 0.414
--- reflecting the deliberate tuning to maximise recall (catching 93.23\% of real
vulnerabilities) while still maintaining high precision (84.92\%). In a security context,
where missing a vulnerability is far more costly than investigating a false alarm, the
lower threshold is the appropriate operational choice.

\section{Agentic Patching Results}

High-confidence alerts from the Gradient Boosting classifier (threshold 0.364) were fed
into the agentic triage layer. The pipeline uses a two-phase LLM remediation approach:
Snippet-Mode (localized $\pm$5-line context) escalating to Full-File Mode when needed.
A Zero-Trust Validation Loop forces every patch to pass both a Java compiler check
(\texttt{mvn clean compile}) and a CodeQL re-scan before being accepted. Key findings:

\begin{itemize}
  \item Alerts with confidence $\geq 0.364$ represented confirmed true positives with
        high fidelity, significantly reducing the number of false alerts reaching the LLM.
  \item The LLM patch generation (Llama-3.3-70b via Groq API) successfully addressed
        SQL Injection, XSS, and Path Traversal patterns in the test cases.
